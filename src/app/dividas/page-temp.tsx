...
export default function DividasPage() {
  ...
  // Estados para edio de valor de parcelas
  ...
  // Verificar dvidas elegveis para converso automtica
  const verificarDividasElegiveis = async () => {
    try {
      ...
      console.error('Erro ao verificar dvidas elegveis:', error);
    }
  };

  // Converter dvida em transao recorrente
  const converterDividaParaRecorrente = async (dividaId: string) => {
    try {
      ...
      console.error('Erro na converso:', error);
      mostrarFeedback('Erro inesperado na converso', 'error');
    } finally {
      setLoading(false);
    }
  };

  // Verificar se dvida j foi convertida para recorrente
  const verificarDividaConvertida = async (nomeCompletoIdDivida: string): Promise<boolean> => {
    try {
      ...
      console.error('Erro ao verificar converso:', error);
    }
    return false;
  };

  // Calcular data prevista de quitao
  ...
  // Funo para calcular estatsticas de uma dvida
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editandoDivida) {
      // Est editando uma dvida existente
      await editarDivida();
    } else {
      // Est criando uma nova dvida
      await criarDivida();
    }
  };
  ...
  const criarDivida = async () => {
    try {
      // Calcular a data da primeira parcela baseado na prxima parcela e parcelas j pagas
      ...
      console.error("Erro ao criar dvida:", error);
    }
  };

  const excluirDivida = async (id: string) => {
    if (!confirm("Tem certeza que deseja excluir esta dvida? Esta ao no pode ser desfeita.")) return;
    ...
    try {
      ...
      if (response.ok) {
        mostrarFeedback("Dvida excluda com sucesso!", "success");
        carregarDados();
      } else {
        const data = await response.json();
        mostrarFeedback(`Erro: ${data.error || "Erro ao excluir a dvida."}`, "error");
      }
    } catch (error) {
      console.error("Erro ao excluir dvida:", error);
      mostrarFeedback("Erro de conexo ao excluir a dvida.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const reativarDivida = async (id: string) => {
    if (!confirm("Tem certeza que deseja reativar esta dvida?")) return;
    ...
    try {
      ...
      if (response.ok) {
        mostrarFeedback("Dvida reativada com sucesso!", "success");
        carregarDados();
      } else {
        const data = await response.json();
        mostrarFeedback(`Erro: ${data.error || "Erro ao reativar a dvida."}`, "error");
      }
    } catch (error) {
      console.error("Erro ao reativar dvida:", error);
      mostrarFeedback("Erro de conexo ao reativar a dvida.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const editarDivida = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Erro ao editar dvida: ${errorData.error || 'Erro desconhecido'}`);
      }
    } catch (error) {
      console.error("Erro ao editar dvida:", error);
      alert("Erro ao editar dvida. Tente novamente.");
    }
  };
  ...
  const marcarParcelaPaga = async (dividaId: string, parcelaId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // Mostrar feedback detalhado sobre a transao criada
        if (data.transacaoCriada) {
          ...
          let mensagem = `Parcela paga! Transao criada: ${valor}`;
          if (data.dividaQuitada) {
            mensagem += "  Dvida quitada completamente!";
          }
  ...
  const salvarNovoValorParcela = async () => {
    ...
    if (valor <= 0 || isNaN(valor)) {
      mostrarFeedback("Valor deve ser um nmero positivo", 'error');
      return;
    }

    console.log(' === INICIANDO SALVAMENTO FRONTEND ===');
    console.log(' Dados:', {
      dividaId: editandoParcela.dividaId,
      parcelaId: editandoParcela.parcelaId,
      valorOriginal: novoValorParcela,
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          mostrarFeedback("Valor atualizado com sucesso!", 'success');
        }
        
        cancelarEdicaoParcela();
        await carregarDados(); // Recarregar os dados
        
        console.log(' === SALVAMENTO CONCLUDO ===');
      } else {
        const error = await response.json();
      ...
      mostrarFeedback("Erro de conexo. Tente novamente.", 'error');
    } finally {
      setLoading(false);
    }
  };

  // Funo para verificar se uma parcela tem valor personalizado
  const parcelaTemValorPersonalizado = (divida: Divida, parcela: ParcelaDivida) => {
    return Math.abs(parcela.valor - divida.valorParcela) > 0.01; // Tolerncia de 1 centavo para flutuao
  };

  // Funo para ordenar parcelas: prximas pendentes primeiro, depois pagas por ltimo
  const ordenarParcelas = (parcelas: ParcelaDivida[]) => {
    return [...parcelas].sort((a, b) => {
      // Primeiro critrio: status (PENDENTE e VENCIDA primeiro, PAGA por ltimo)
      const statusOrder = { 'PENDENTE': 0, 'VENCIDA': 1, 'PAGA': 2 };
      if (statusOrder[a.status] !== statusOrder[b.status]) {
        return statusOrder[a.status] - statusOrder[b.status];
      }
      
      // Para parcelas PENDENTES e VENCIDAS: ordem crescente (prximas primeiro)
  ...
  // Funo para obter informaes da prxima parcela a ser paga
  const obterProximaParcela = (divida: Divida) => {
    ...
    const parcelasAPagar = divida.parcelas
      ...
      .sort((a, b) => a.numero - b.numero); // Ordenar por nmero crescente
    
    // Retornar a primeira da lista (menor nmero = prxima a ser paga)
    return parcelasAPagar[0] || null;
  };

  // Funo para formatar valores monetrios
  ...
  // Funo para formatar valores de forma compacta
  ...
  // === BLOCO DE RENDERIZAO PRINCIPAL ===

  // Se ainda est carregando e no h dados, mostra loading
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${bgClass} relative overflow-hidden`}>
      
      {/* Notificao de Feedback */}
      {mensagemFeedback && (
      ...
      {/* Boto Dark Mode - Responsivo e no sobreposto */}
      <button
      ...
      <div className="fixed bottom-6 right-6 z-40">
        <HelpButton 
          ...
          steps={[
            {
              title: "Dvidas",
              content: "Gerencie suas dvidas parceladas e acompanhe o progresso de pagamento."
            },
            {
              title: "Como usar",
              content: " Cadastre dvidas parceladas\n Marque parcelas como pagas\n Acompanhe estatsticas\n Veja insights inteligentes"
            },
            {
              title: "Personalizar Valores",
              content: " Clique no cone de edio ao lado do valor\n Altere o valor de parcelas individuais\n Apenas parcelas pendentes podem ser editadas\n O valor total da dvida  recalculado automaticamente"
            }
          ]}
        />
      </div>
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-white/90 text-lg font-medium">
                    Controle suas dvidas parceladas
                  </p>
                </div>
              </div>
        ...
        {/* Dvidas Elegveis para Converso Automtica */}
                ...
                <div>
                  <h3 className={`text-lg font-bold ${
                    darkMode ? 'text-white' : 'text-gray-900'
                     ...
                     Converso para Recorrente Disponvel
                  </h3>
                  <p className={`text-sm ${
                    darkMode ? 'text-purple-300' : 'text-purple-700'
                  }`}>
                    {dividasElegiveis.length} dvida(s) podem virar recorrentes (mantendo a dvida original)
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className={`p-3 rounded-lg border-l-4 text-sm ${
                  ...
                  <div className="flex items-center gap-2 font-medium">
                    <Info size={14} />
                    Como funciona a converso automtica?
                  </div>
                  <div className="text-xs mt-2 opacity-90">
                     Qualquer dvida com parcelas pendentes pode ser convertida em recorrente<br/>
                     A dvida original continua aparecendo na lista de dvidas<br/>
                     Uma transao recorrente  criada para gerar os pagamentos automaticamente<br/>
                     Funciona para qualquer quantidade de parcelas (1, 15, 30, etc.)<br/>
                     A recorrente ter o mesmo valor e categoria da dvida
                  </div>
                </div>
              </div>
            )}
            ...
            {dividas.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className={`text-xl font-semibold mb-2 ${
                  ...
                  Nenhuma dvida encontrada
                </h3>
                <p className={`text-base mb-6 ${
                  darkMode ? 'text-gray-400' : 'text-gray-600'
                }`}>
                  {filtroStatus === 'TODAS' 
                    ? 'Cadastre sua primeira dvida!'
                    : `Nenhuma dvida "${filtroStatus.toLowerCase()}" encontrada.`
                  }
                </p>
                {filtroStatus === 'TODAS' && (
                  <button
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {dividas.map((divida) => (
                  ...
                  >
                    ...
                    {/* Informaes da dvida - Design Minimalista */}
                    <div className="space-y-3">
                      {/* Progresso Principal */}
                      ...
                      {/* Prxima Parcela e Data Prevista de Quitao */}
                      {(() => {
                        ...
                        const elegiveParaConversao = parcelasRestantes > 0 && 
                          dividasElegiveis.some(elegivel => elegivel.id === divida.id); // S exibe se est na lista de elegveis
                              ...
                              <div className={`p-3 rounded-lg border-l-4 ${
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <button
                                    ...
                                    title="Converter para transao recorrente"
                                  >
                                     Converter
                                  </button>
                                </div>
                        ...
                        <h4 className={`font-bold mb-4 flex items-center gap-2 ${
                          ...
                          <span className={`text-xs font-normal ${
                            darkMode ? 'text-gray-400' : 'text-gray-500'
                             ...
                             Prximas primeiro, pagas por ltimo
                          </span>
                        </h4>
                                ...
                                <div className="flex items-center justify-between">
                                  <div className="flex-1">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className={`p-3 rounded-lg ${
                                          ...
                                          <div className="flex items-center justify-between mb-2">
                                            ...
                                            {parcelaTemValorPersonalizado(divida, parcela) && (
                                              <span 
                                                className="px-2 py-1 bg-gradient-to-r from-amber-500 to-yellow-500 text-white text-xs font-bold rounded-full"
                                          ...
                                          {editandoParcela?.dividaId === divida.id && editandoParcela?.parcelaId === parcela.id ? (
                                            <div className="space-y-2">
                                              <div className="relative">
                                                ...
                                                <input
                                                  ...
                                                  className={`w-full pl-8 pr-4 py-2 rounded-lg border-2 font-bold text-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all ${
                                                    darkMode 
                                              ...
                                              <div className="flex gap-2">
                                                <button
                                                  ...
                                                  disabled={loading}
                                                  className="flex-1 px-4 py-2 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 disabled:opacity-50 text-white font-bold rounded-lg transition-all"
                                                >
                                                  {loading ? '' : ''} Salvar
                                                </button>
                                                <button
                                                  onClick={cancelarEdicaoParcela}
                                                  className="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white font-bold rounded-lg transition-colors"
                                                >
                                            ...
                                            <div className="flex items-center justify-between">
                                              ...
                                              {/* Boto de editar - apenas para parcelas pendentes */}
                                              {parcela.status === 'PENDENTE' && (
                                                <button
                                                  onClick={() => iniciarEdicaoParcela(divida.id, parcela.id, parcela.valor)}
                                                  className="px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors font-medium text-sm"
                                                  title="Editar valor"
                                                >
                                                   Editar
                                                </button>
                                      ...
                                      {/* Boto de Pagar - apenas para parcelas pendentes */}
                                      {parcela.status === 'PENDENTE' && (
                                        <button
                                          onClick={() => marcarParcelaPaga(divida.id, parcela.id)}
                                          className="w-full px-6 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold rounded-xl transition-all duration-200 hover:scale-105 flex items-center justify-center gap-2 shadow-lg"
                                        >
                                          <Check size={20} />
                                          Marcar como Paga
                                        </button>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-2xl font-bold text-white drop-shadow-sm">
                        {editandoDivida ? ' Editar Dvida' : ' Nova Dvida'}
                      </h2>
                      <p className="text-white/90 text-sm font-medium">
                        {editandoDivida ? 'Edite os dados da dvida parcelada' : 'Cadastre uma nova dvida parcelada'}
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      placeholder="Ex: Financiamento do carro, Carto de crdito..."
                      required
                    />
                  </div>
                  ...
                  {/* Parcelas j pagas e prxima data */}
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-3">
                      <label className={`flex items-center gap-2 text-base font-bold transition-colors duration-300 ${
                        darkMode ? 'text-white' : 'text-gray-800'
                  ...
                  <div className="flex gap-4 pt-6">
                    ...
                    >
                      {editandoDivida ? 'Salvar Alteraes' : 'Criar Dvida'}
                    </button>
                  </div>
                </form>
              </div>
